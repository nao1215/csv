...
`
	...
	t.Run("validate eqfield", func(t *testing.T) {
		t.Parallel()
...
`
		...
		list := make([]password, 0)
		errs := c.Decode(&list)
		for i, err := range errs {
			switch i {
			case 0:
				if err.Error() != "line:3 column password_confirm: field is not equal to the specified field: field=PasswordConfirm, other=Password" {
					t.Errorf("CSV.Decode() got errors: %v", err)
				}
			}
		}
...
`
		...
		type row struct {
			Text   string `validate:"fieldcontains=Needle"`
			Needle string
		}
		...
		for i, err := range errs {
			switch i {
			case 0:
				if err.Error() != "line:3 column text: field does not contain the specified field value: field=Text, other=Needle" {
					t.Errorf("CSV.Decode() got errors: %v", err)
				}
			}
		}
	...
	t.Run("validate fieldexcludes", func(t *testing.T) {
		t.Parallel()
...
`
		...
		type row struct {
			Text   string `validate:"fieldexcludes=Needle"`
			Needle string
		}
		...
		for i, err := range errs {
			switch i {
			case 0:
				if err.Error() != "line:2 column text: field contains the prohibited field value: field=Text, other=Needle" {
					t.Errorf("CSV.Decode() got errors: %v", err)
				}
			}
		}
...
`
		...
		type row struct {
			Value int `validate:"gtefield=Limit"`
			Limit int
		}
		...
		for i, err := range errs {
			switch i {
			case 0:
				if err.Error() != "line:3 column value: field is not greater than or equal to the specified field: field=Value, other=Limit" {
					t.Errorf("CSV.Decode() got errors: %v", err)
				}
			}
		}
...
`
	...
	t.Run("validate ltefield", func(t *testing.T) {
		t.Parallel()
...
`
		...
		type row struct {
			Value int `validate:"ltefield=Limit"`
			Limit int
		}
		...
		for i, err := range errs {
			switch i {
			case 0:
				if err.Error() != "line:3 column value: field is not less than or equal to the specified field: field=Value, other=Limit" {
					t.Errorf("CSV.Decode() got errors: %v", err)
				}
			}
		}
...
`
		...
		type row struct {
			Value int `validate:"ltfield=Limit"`
			Limit int
		}
		...
		for i, err := range errs {
			switch i {
			case 0:
				if err.Error() != "line:3 column value: field is not less than the specified field: field=Value, other=Limit" {
					t.Errorf("CSV.Decode() got errors: %v", err)
				}
			}
		}
